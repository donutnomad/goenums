...
func TestToInt64(t *testing.T) {
	tests := []struct {
		...
		{"int8 negative", int8(-128), -128, true},
		{"int16 positive", int16(32767), 32767, true},
		{"int16 negative", int16(-32768), -32768, true},
		{"int32 positive", int32(2147483647), 2147483647, true},
		{"int32 negative", int32(-2147483648), -2147483648, true},
		{"int64 positive", int64(9223372036854775807), 9223372036854775807, true},
		{"int64 negative", int64(-9223372036854775808), -9223372036854775808, true},
		{"int positive", int(123456), 123456, true},
		{"int negative", int(-123456), -123456, true},

		// 无符号整数
		{"uint8", uint8(255), 255, true},
		{"uint16", uint16(65535), 65535, true},
		{"uint32", uint32(4294967295), 4294967295, true},
		{"uint64 small", uint64(123456), 123456, true},
		{"uint64 large", uint64(9223372036854775807), 9223372036854775807, true},
		{"uint", uint(123456), 123456, true},

		// 不支持的类型
		{"float32", float32(3.14), 0, false},
...
func TestToFloat64(t *testing.T) {
	tests := []struct {
		...
		{"float64", float64(3.141592653589793), 3.141592653589793, true},
		{"float32 negative", float32(-2.71), float64(float32(-2.71)), true},
		{"float64 negative", float64(-2.718281828), -2.718281828, true},
		{"float32 zero", float32(0), 0, true},
		{"float64 zero", float64(0), 0, true},
		{"float32 infinity", float32(math.Inf(1)), math.Inf(1), true},
		{"float64 infinity", math.Inf(1), math.Inf(1), true},
		{"float32 NaN", float32(math.NaN()), math.NaN(), true},
			...
			if ok {
				if math.IsNaN(tt.expected) {
					if !math.IsNaN(result) {
						t.Errorf("toFloat64(%v) = %f, want NaN", tt.input, result)
					}
...
// 测试 parseFloat64Value 函数
func TestParseFloat64Value(t *testing.T) {
	t.Run("整数类型转换", func(t *testing.T) {
		var intVal int
		err := parseFloat64Value(42.7, &intVal)
		if err != nil {
			t.Errorf("parseFloat64Value failed: %v", err)
		}
		if intVal != 42 {
		...
		// 测试所有整数类型
		var int8Val int8
		err = parseFloat64Value(-123.9, &int8Val)
		if err != nil {
			t.Errorf("parseFloat64Value failed: %v", err)
		}
		if int8Val != -123 {
		...
		var int16Val int16
		err = parseFloat64Value(32000.5, &int16Val)
		if err != nil {
			t.Errorf("parseFloat64Value failed: %v", err)
		}
		if int16Val != 32000 {
			t.Errorf("expected 32000, got %d", int16Val)
		}

		var int32Val int32
		err = parseFloat64Value(-2147483000.7, &int32Val)
		if err != nil {
			t.Errorf("parseFloat64Value failed: %v", err)
		}
		if int32Val != -2147483000 {
			t.Errorf("expected -2147483000, got %d", int32Val)
		}

		var int64Val int64
		err = parseFloat64Value(123456789.0, &int64Val)
		if err != nil {
			t.Errorf("parseFloat64Value failed: %v", err)
		}
		if int64Val != 123456789 {
			t.Errorf("expected 123456789, got %d", int64Val)
		}

		// 测试无符号整数类型
		var uintVal uint
		err = parseFloat64Value(12345.8, &uintVal)
		if err != nil {
			t.Errorf("parseFloat64Value failed: %v", err)
		}
		if uintVal != 12345 {
		...
		var uint8Val uint8
		err = parseFloat64Value(255.9, &uint8Val)
		if err != nil {
			t.Errorf("parseFloat64Value failed: %v", err)
		}
		if uint8Val != 255 {
		...
		var uint16Val uint16
		err = parseFloat64Value(65535.1, &uint16Val)
		if err != nil {
			t.Errorf("parseFloat64Value failed: %v", err)
		}
		if uint16Val != 65535 {
		...
		var uint32Val uint32
		err = parseFloat64Value(4294967000.0, &uint32Val)
		if err != nil {
			t.Errorf("parseFloat64Value failed: %v", err)
		}
		if uint32Val != 4294967000 {
			t.Errorf("expected 4294967000, got %d", uint32Val)
		}

		var uint64Val uint64
		err = parseFloat64Value(987654321.0, &uint64Val)
		if err != nil {
			t.Errorf("parseFloat64Value failed: %v", err)
		}
		if uint64Val != 987654321 {
			t.Errorf("expected 987654321, got %d", uint64Val)
		}
	})

	t.Run("浮点数类型转换", func(t *testing.T) {
		var float32Val float32
		err := parseFloat64Value(3.14159, &float32Val)
		if err != nil {
			t.Errorf("parseFloat64Value failed: %v", err)
		}
		if float32Val != 3.14159 {
			t.Errorf("expected 3.14159, got %f", float32Val)
		}

		var float64Val float64
		err = parseFloat64Value(-2.718281828, &float64Val)
		if err != nil {
			t.Errorf("parseFloat64Value failed: %v", err)
		}
		if float64Val != -2.718281828 {
	...
	t.Run("JSON类型转换", func(t *testing.T) {
		// parseFloat64Value对于复杂类型会将数字转换为JSON字符串再解析
		// 但这个测试可能不太合理，因为42.5无法直接JSON化为一个结构体
		// 我们测试一个更合理的场景：将42.5序列化为"42.5"然后解析
		type TestStruct struct {
		...
		err := parseFloat64Value(42.5, &floatVar)
		if err != nil {
			t.Errorf("parseFloat64Value failed: %v", err)
		}
		if floatVar != 42.5 {
			t.Errorf("expected 42.5, got %f", floatVar)
		}

		// 测试通过JSON解析复杂类型 - 这会将 42.5 转换为 "42.5" 然后尝试JSON解析
		var testStruct TestStruct
		err = parseFloat64Value(42.5, &testStruct)
		// 这应该失败，因为 "42.5" 不是有效的JSON对象
		if err == nil {
			t.Errorf("parseFloat64Value should fail when converting float to struct via JSON")
		}

		// 这个测试可能不是很有意义，因为parseFloat64Value主要是为了类型转换
		// 复杂类型的JSON解析应该在其他函数中测试
		t.Logf("parseFloat64Value主要用于基础类型转换，复杂类型测试跳过")
	})
}
